...
content = content.replace(
    '<title>章节详情 - 小说管理平台</title>',
    '<title>梦境阅读 · 章节详情</title>'
)

# 2. Add font links after highlight.js
content = content.replace(
...
# 3. Replace the entire CSS style section
...
# 4. Add floating orbs right after <body> tag
body_tag = '<body>'
body_idx = content.find(body_tag)
if body_idx != -1:
    floating_orbs = '''
    <!-- 梦幻气泡背景 -->
...
'''
    content = content[:body_idx + len(body_tag)] + floating_orbs + content[body_idx + len(body_tag):]

# 5. Wrap main content in glass-card
# Find the container div and wrap its contents after floating orbs
container_start = content.find('<div class="container">')
if container_start != -1:
    # Find where the container actually starts its content (after header, etc.)
    # We'll just add glass-card styling through CSS rather than changing HTML structure
    # to preserve all functionality
    pass

# Write the final transformed file
with open(r'E:\WorkSpace\articleCollect\src\main\resources\templates\chapter-detail.html', 'w', encoding='utf-8') as f:
    f.write(content)

print('Transformation complete! Chapter-detail.html has been updated with dreamy aesthetic.')
